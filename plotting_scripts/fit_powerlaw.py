...
def log_likelihood(params, r, errors, *indep_vars, priors=True):
    ...
    scatter = params[-1]
    if priors:
        if scatter <= 0 or scatter > 10:
            return -np.inf  # prior on size of scatter in dex
        if np.any(np.abs(coeffs) > 10):
            return -np.inf  # prior on slopes (nothing steeper than 10)
        if np.any(np.abs(params[0]) > 100):
            return -np.inf
    norm = params[0]  # normalization of relation
    r_med = norm + np.inner(
        coeffs, np.c_[indep_vars]
    )  # median value predicted by relation
    dlogr = r - r_med  # differences from median - should be log-normal
    lnprob = np.log(
        1
        / np.sqrt(2 * np.pi * (errors**2 + scatter**2))
        * np.exp(-(dlogr**2) / (2 * (errors**2 + scatter**2)))
    )  # likelihoods of deviations, given the scatter
    # lnprob += np.log(weights)
    if np.any(np.invert(np.isfinite(lnprob))):
        return -np.inf
...
def log_likelihood_variable_calibration(
    params, r, errors, calibration_sigma, *indep_vars, priors=True
):
    if errors is None:
        errors = np.zeros_like(r)
    ...
    r_med = calibration * (
        norm + np.inner(coeffs, np.c_[indep_vars])
    )  # median value predicted by relation
    dlogr = r - r_med  # differences from median - should be log-normal
    lnprob = np.log(
        1
    ...
    lnprob += np.log(
        1
        / np.sqrt(2 * np.pi * calibration_sigma**2)
        * np.exp(-(calibration**2) / (2 * calibration_sigma**2))
    )  # lognormal calibration to allow for variable slope
    # lnprob += np.log(weights)
    if np.any(np.invert(np.isfinite(lnprob))):
        return -np.inf
    if np.any(np.isnan(lnprob)):
...
def fit_powerlaw_model_with_scatter(
    dep_var,
    *indep_vars,
    errors=None,
    chainlength=10000,
    ...
    if calibration_sigma == 0:
        p0 = np.array([b, *coeffs, scatter_guess]) + 1e-2 * np.random.normal(
            size=(nwalkers, ndim)
        )
        sampler = emcee.EnsembleSampler(
            nwalkers, ndim, log_likelihood, args=(dep_var, errors, *indep_vars)
        )
    else:
        p0 = np.array(
            [b, calibration_sigma, *coeffs, scatter_guess]
        ) + 1e-2 * np.random.normal(size=(nwalkers, ndim))
        sampler = emcee.EnsembleSampler(
            nwalkers,
            ndim,
            log_likelihood_variable_calibration,
            args=(dep_var, errors, calibration_sigma, *indep_vars),
        )
    ...
    if not return_likelihood or return_DIC:
        return samples
    stuff_toreturn = [
        samples,
    ]
    if return_likelihood:
        med = samples[
            sampler.get_log_prob(discard=burnin, flat=True, thin=100).argmax()
        ]
        if calibration_sigma == 0:
            func = lambda x: -log_likelihood(
                x, dep_var, errors, *indep_vars, priors=False
            )
        else:
            func = lambda x: -log_likelihood_variable_calibration(
                x, dep_var, errors, calibration_sigma, *indep_vars, priors=False
            )
    ...
    if return_DIC:
        ...
        if calibration_sigma == 0:
            DIC = 2 * np.average(-2 * logprobs) + 2 * log_likelihood(
                np.mean(samples, axis=0), dep_var, errors, *indep_vars
            )
        else:
            DIC = 2 * np.average(
                -2 * logprobs
            ) + 2 * log_likelihood_variable_calibration(
...
def fit_powerlaw_model_with_variable_scatter(
    dep_var, time, *indep_vars, chainlength=10000, errors=None
):
    if errors is None:
        errors = np.zeros_like(dep_var)
    ndim = 3 + len(
        indep_vars
    )  # normalization + slope for each variable + scatter slope + scatter intercept
    nwalkers = 40

    coeffs, b, rms_error = multilinear_fit(dep_var, *indep_vars)
    ypred = np.inner(coeffs, np.c_[indep_vars]) + b
    scatter_intercept_guess = np.std(dep_var - ypred)
    scatter_slope_guess = 0.0
    if np.abs(b) > 10:
        b = 0
        coeffs[:] = 0

    p0 = np.array(
        [b, *coeffs, scatter_intercept_guess, scatter_slope_guess]
    ) + 1e-2 * np.random.normal(size=(nwalkers, ndim))

    sampler = emcee.EnsembleSampler(
        nwalkers,
        ndim,
        log_likelihood_variable_scatter,
        args=(dep_var, time, errors, *indep_vars),
    )
    burnin = max(200, chainlength // 10)
    sampler.run_mcmc(p0, chainlength, progress=False)
    samples = sampler.get_chain(discard=burnin, flat=True, thin=100)
...
def log_likelihood_orth(params, x, y, dx, dy):
    ...
    logL = np.log(
        (2 * np.pi * sigma_eff2) ** -0.5 * np.exp(-(delta_i**2) / (2 * sigma_eff2))
    )  # -0.5*np.log(2*np.pi*(sigma_i**2 + scatter_orth**2)) - delta_i**2/(2*(sigma_i**2 + scatter_orth**2))
...
def orthogonal_linear_regression(x, y, dx=None, dy=None, chainlength=1000):
    ...
    p0 = np.array([theta, bperp, logscatter]) + 1e-2 * np.random.normal(
        size=(nwalkers, ndim)
    )
    ...
    sampler = emcee.EnsembleSampler(
        nwalkers, ndim, log_likelihood_orth, args=(x, y, dx, dy)
    )
    sampler.run_mcmc(p0, chainlength, progress=False)
    samples = sampler.get_chain(discard=burnin, flat=True, thin=100)

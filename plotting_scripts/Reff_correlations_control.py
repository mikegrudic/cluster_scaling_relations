...
for g in np.unique(galaxies):
    ...
    for n, i in enumerate(independent_variables):
        if np.all(np.isnan(i[idx])):
            ...
            x, y = x[idx2], y[idx2]
            y -= np.mean(y)
            logm = np.log10(m[idx][idx2])
            logm -= logm.mean()
            logtau = np.log10(ages[idx][idx2])
            logtau -= logtau.mean()
            p_m, p_tau = np.linalg.lstsq(np.c_[logm, logtau], np.array(y))[0]
            if g == "M82":  # ages for M82 are very very approximate, leave out of fit
                p_tau = 0
                p_m = np.polyfit(logm, y, 1)[0]
            y = y - p_m * logm - p_tau * logtau
            #            np.log10(r[idx]),np.log10(m[idx])) #            idx2 = np.isfinite(x)*np.isfinite(y)
    ...
    for j in range(corrs.shape[1]):
        ...
        if np.isfinite(ps[i, j]):
            ax.text(
                j,
                i,
                r"$%2.2g\sigma$" % p_to_sigma(ps[i, j]),
                fontsize=6,
...
cbar.set_label(r"Correlation with $R_{\rm eff}/M^p \tau^q$", size=8)
ax.set_yticks(range(len(gals)))
ax.set_yticklabels(
    [
        (
            r"$\mathbf{%s}$" % str.upper(g) + r" ($N=%d$)" % n
            if (g in PHANGS_LEGUS or g == "PHANGS+LEGUS")
...
ax.set_xticklabels([""] + itags, rotation=90, fontsize=6)
plt.savefig("Reff_correlations_control.pdf", bbox_inches="tight")
